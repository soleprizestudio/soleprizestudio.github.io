...
* **위젯에 2·3등이 안 뜨던 문제**: 2·3등 데이터 로드가 앱 내 광고 잠금 뒤에 묶여 있었다. 로드는 잠금과 무관하게 항상 하고, 앱 화면 표시만 잠그도록 분리 (위젯은 광고와 무관하게 표시).
* **온라인 판매점에 거리가 뜨던 문제**: "동행복권"(온라인 전용) 판매점을 후보군 저장 단계에서 제외. 온라인 판별 기준을 `LotteryStore.isOnlineSeller` 한 곳으로 모았다.
* **회차 표시 불일치**: 새 회차 발표 전, 목록은 이전 회차인데 상단 회차 번호만 최신으로 뜨던 문제를 목록·회차 표시가 일치하도록 수정.

## 에필로그 — 배포하며 배운 것

세 번 뒤집은 위젯 위치 문제가 결국 가르쳐준 건 하나다 — "안 된다"의 원인이 코드가 아니라 내 테스트 환경일 수 있다. 설계를 갈아엎기 전에, 조건을 통제하고 재현부터 하자. 초심자가 치러야 할 큰 비용 중 하나였다고 기억해야겠다.
