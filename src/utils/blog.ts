...
const getNormalizedPost = async (post: CollectionEntry<'post'>): Promise<Post> => {
  ...
  const {
    // Permalinks are slug- (filename-) based, not date-based, so a missing
    // publishDate only affects display/sort order - defaulting to "now" is
    // fine here and never used to fill in a stable date.
